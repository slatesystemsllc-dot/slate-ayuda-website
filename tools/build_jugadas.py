...
(function(){
  ...
  if(slot && conf){
    ...
    var html='<h3>Tu ruta</h3><div class="rt">Las jugadas de '+key+', en orden</div>';
    if(conf.especial){ html+='<div class="esp"><b>Solo tu oficio puede hacer esto:</b> '+conf.especial+'</div>'; }
    html+='<ol>'+ids.map(function(id){ return '<li><a href="'+n[id]+'.html">'+t[id]+'</a></li>'; }).join('')+'</ol>';
    html+='<p style="font-size:13px;opacity:.85">Después de estas cinco, sigue con las demás. Todas están abajo.</p>';
    slot.innerHTML=html; slot.hidden=false;
  }
  var nt=document.getElementById('oficio-note'); if(nt && key){ nt.textContent='Esta guía está en modo '+key+'.'; nt.hidden=false; }
})();
</script>
"""
